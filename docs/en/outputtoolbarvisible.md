---
title: "Outputtoolbarvisible"
sidebar_label: "Outputtoolbarvisible"
---

## OutputToolbarVisible (Statement)

### Format

**outputtoolbarvisible** ( [boolean_expression](./booleanexpressions.md) )\
**outputtoolbarvisible** [boolean_expression](./booleanexpressions.md)

### Description

Sets the view option for the tool bar on the Text Output portion of the window.

### Example

    outputtoolbarvisible 0
    pause 2
    outputtoolbarvisible 1

### See Also

[EditVisible](./editvisible.md), [GraphToolBarVisible](./graphtoolbarvisible.md), [GraphVisible](./graphvisible.md), [Include](./include.md),[MainToolbarVisible](./maintoolbarvisible.md), [OutputToolBarVisible](./outputtoolbarvisible.md), [OutputVisible](./outputvisible.md), [RegexMinimal](./regexminimal.md), [Ostype](./ostype.md), [System](./system.md), [Version](./version.md)

### New To Version

2.0.0.4
