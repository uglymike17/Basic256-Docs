...
Note: To speed things up for children doing more advanced programs, the [FASTGRAPHICS](./fastgraphics.md) mode has been added. When in this mode, the display will only be updated when the BASIC program uses the [REFRESH](./refresh.md) command. This allows kids to benefit from double-buffering without having to understand it completely.
